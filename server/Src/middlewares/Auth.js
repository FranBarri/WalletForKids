...
router.post('/loginKids', async (req, res) => {
  ...
  if (!user) {
      return res.status(401).json({ message: 'User not found', data: null, status: 401 });
  }
  ...
  if(req.body.password !== decryptedPassword){
      return res.status(401).json({ message: 'Invalid username or password',data:null,status:401});
  }
